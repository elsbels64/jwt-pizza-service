...
class DB {
  ...
  async listUsers(offset = 0, limit = 10, nameFilter = '*') {
  const connection = await this.getConnection();
  try {
    const off = Number(offset) || 0;
    const lim = Math.max(1, Number(limit) || 10);
    const name = nameFilter.replace(/\*/g, '%');
    const users = await this.query(
      connection,
      `SELECT id, name, email FROM user WHERE name LIKE ? ORDER BY id LIMIT ${off}, ${lim}`,
      [name]
    );
 ...
 async addDinerOrder(user, order) {
  ...
  async query(connection, sql, params) {
    ...
    logger.log('info', 'db', { query: sql, duration: `${duration}ms`});
    return results;
  }
